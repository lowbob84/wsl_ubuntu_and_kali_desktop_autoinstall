...
# description             : "Automatic install of wsl ubuntu and ubuntu 22.04 with gnome desktop"
...
export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games:/usr/local/games:/snap/bin:/opt/cxoffice/bin:/snap/bin
...
cat <<EOF > "/home/${USER}/start_ubuntu_dekstop.sh"
#!/bin/bash
if [ -z "$DISPLAY" ]; then
...
chmod 755 "/home/${USER}/start_ubuntu_dekstop.sh"
cd "/home/${USER}"
...
N=5; while [[ $((--N)) >  0 ]]; do  echo -e "\033[2J\033[0m"; echo "Run start_ubuntu_dekstop.sh after reboot in $N seconds" |  figlet -c && sleep 1 ; done
